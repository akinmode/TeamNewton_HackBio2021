...
echo "CHECKING FOR REQUIRED DEPENDECIES ..."
	REQ_DEPEND=("bash" "python3" "julia" "Rscript")
echo "[INFO] The Following packages are required to execute the submitted scripts: "${REQ_DEPEND[@]}
	for i in "${REQ_DEPEND[@]}";do
	    command -v $i >/dev/null 2>&1 || {
	        echo >&2 "[WARNING] $i package is required to run the submitted scripts";
			echo "[WARNING] Install $i package and rerun";
	        exit 1;
	    }
		echo "[CHECK] $i package already installed!";
	done
echo " "
echo "[SUCCESS] All required packages are available on local machine"
echo " "
echo "==============================================================="
